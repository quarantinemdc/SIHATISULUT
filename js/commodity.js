document.addEventListener('DOMContentLoaded', () => {
    ...
    const komoditasData = {
        "koi": {
            "nama": "Ikan Koi",
            "deskripsi": "Pemasukan atau pengeluaran media pembawa sebagai barang bawaan dilaksanakan dengan ketentuan:",
            "persyaratan": [
    ...
    cards.forEach(card => {
        card.addEventListener('click', () => {
            ...
            if (data) {
                // Menggabungkan persyaratan dan ketentuan menjadi satu daftar jika diperlukan
                let combinedList = [];
                if (data.persyaratan) {
                    combinedList = combinedList.concat(data.persyaratan);
                }
                if (data.ketentuan) {
                    combinedList = combinedList.concat(data.ketentuan);
                }

                const listHtml = combinedList.map(item => `<li>${item}</li>`).join('');
